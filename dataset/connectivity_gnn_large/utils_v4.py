# v4: add more node and edge to the graph (include whole of bay area)
...
def plot_reg_graph(img_dir, img_idx, edge_list, values, node_ID, edge_labels, plot_norm, img_name):
    ...
    nx.draw(G, pos, cmap=plt.cm.Blues, node_color=values, \
            with_labels=True, node_size=600, font_size=17, font_color='red', font_weight='bold', width=2, vmin=plot_norm.vmin, vmax=plot_norm.vmax*1.3)
    sm = plt.cm.ScalarMappable(cmap=plt.cm.Blues, norm=plot_norm)
    position=fig1.add_axes([0.9,0.4,0.04,0.5])
...
def plot_reg_graph_new(img_dir, img_idx, edge_list, values, node_ID, edge_labels, plot_norm, img_name):
    ...
    bridge_list = [(0, 1), (0, 5), (0, 69), (1, 2), (1, 5), (2, 3), (3, 4), \
            ...
            (77, 78), (77, 79), (78, 79), (80, 81), (52, 85), (57, 98), (58, 97), \
            (83, 84), (84, 85), (84, 86), (86, 87), (87, 89), (88, 89), (88,101), \
            (89, 99), (89,100), (90,102), (91, 92), (91, 94), (92, 93), (92, 94), \
            (92, 95), (93, 95), (94, 96), (95, 98), (96, 97), (101, 102)]
    ...
    fig1 = plt.figure()
    ax = plt.axes()
    img = matplotlib.image.imread('map2.png')
    
    plt.imshow(img, extent=[-122.5500, -121.6500, 37.1500, 37.8700])
    plt.xlim((-122.5500, -121.6500))
    plt.ylim((37.1500, 37.8700))
    
    pos = nx.get_node_attributes(G,'pos')
    nodes = nx.draw_networkx_nodes(G, pos, cmap=plt.cm.Blues, node_color=values, edgecolors='k', \
            linewidths=1, node_size=50, vmin=plot_norm.vmin, vmax=plot_norm.vmax)
    nx.draw_networkx_edges(G, pos, width=3.0, alpha=0.25)
    sm = plt.cm.ScalarMappable(cmap=plt.cm.Blues, norm=plot_norm)
    position=fig1.add_axes([0.82,0.2,0.04,0.5])
    plt.colorbar(sm, cax=position)
    # plt.axis('equal')
    plt.subplots_adjust(top=1.0, bottom=0, right=1.0, left=0.0, hspace=0, wspace=0)
    plt.savefig(img_dir+"/path_reg_{}_{}.jpg".format(img_name, img_idx))
    plt.close('all')
...
def cal_bridge_info():
    SpreadSheet = np.genfromtxt('bridge_info_v4.csv', delimiter=',', dtype=None, encoding='utf-8-sig')
    colNames = SpreadSheet[0, :]
    Data = SpreadSheet[1:, :]
...
def load_data_case(idx, num_sample, dataset_name, imageset_name):
    ...
    road_list = [(1,6), (2,6), (9,12), (13,20), (13,21), (17,23), \
            ...
            (65, 66), (66, 72), (67, 70), (69, 70), (81, 82), (82, 83), \
            (85, 101), (100,101), (90,91)]
    # compatiable with bridge_info_v4
    target_node_ID = idx
    node_thre = [0.75]
    node_class = len(node_thre) + 1
    N_iter = 10000
    data_dir, img_dir = './{}/data_{}_v4'.format(dataset_name, target_node_ID), './{}/img_{}_v4'.format(imageset_name, target_node_ID) 
