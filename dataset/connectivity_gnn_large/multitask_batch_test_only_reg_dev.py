...
class BridgeDataset(DGLDataset):
    def __init__(self, start_id, stop_id, data_dir):
        self.start_id = start_id
        self.stop_id = stop_id
        self.num_sample = stop_id - start_id
        self.data_dir = data_dir
        if type(stop_id) == list:
            super(BridgeDataset, self).__init__(name='bridge', hash_key={stop_id[0], data_dir[0]})
        else:
            super(BridgeDataset, self).__init__(name='bridge', hash_key={stop_id, data_dir})
    ...
    def process(self):
        ...
        for num_sample, dir in zip(num_sample_list, data_dir_list):
            ...
            graph_id_range = [i+self.start_id for i in range(num_sample)]
...
def test_multitask_record(dataset, target_node_id, model, start_id, stop_id, val_data_dir, val_img_dir, \
    ...
    graph_list = [dataset[g_id].to(device) for g_id in range(stop_id-start_id)]
    ...
    return np.vstack(single_summary), np.vstack(single_reg_result)
...
test_targets = [14] #reserve_sample(bridge_list, road_list, n_node)
...
checkpoint = torch.load('./saved_model_dev/{}_{}_{}_{}.pth'.format(model_idx, batch_size, dataset_name, percentage), map_location=device)
...
for target_node_id in test_targets:
    if target_node_id in train_targets:
        val_dir = './{}/data_{}_v4'.format(dataset_name, target_node_id)    # path to load different data
        img_dir = './{}/img_reg_{}_v4'.format(imageset_name, target_node_id)  # path to save the prediction
