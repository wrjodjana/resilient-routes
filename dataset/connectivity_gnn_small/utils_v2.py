...
def create_plot_all(prediction, prediction_cla, gt_cla, data_dir, img_dir, img_idx, num_class):
    ...
    if 1:
        plot_reg_graph_new(img_dir, img_idx, edge_list, ground_truth, \
            node_ID, edge_labels, plt.Normalize(vmin = 0.0, vmax=1.0), "before")
        plot_reg_graph_new(img_dir, img_idx, edge_list, prediction, \
            node_ID, edge_labels, plt.Normalize(vmin = 0.0, vmax=1.0), "after")
        plot_reg_graph_new(img_dir, img_idx, edge_list, np.abs(prediction - ground_truth), \
            node_ID, edge_labels, plt.Normalize(vmin = 0.0, vmax=0.05), "diff")
    if 1:
        # G = nx.Graph()
        # G.add_nodes_from(node_ID)
        # G.add_edges_from(edge_list)
        ...
        plot_cla_graph_new(img_dir, img_idx, graph_data, edge_list, node_ID, edge_labels, 'before', gt_cla/(num_class-1), num_class)

        plot_cla_graph_new(img_dir, img_idx, graph_data, edge_list, node_ID, edge_labels, 'after', prediction_cla/(num_class-1), num_class)
...
def plot_cla_graph(img_dir, img_idx, graph_data, edge_list, node_ID, edge_labels, img_name, values, num_class):
    ...
    # nx.draw(G, pos, cmap=plt.get_cmap('viridis'), node_color=values, \
    #         with_labels=True, node_size=800, font_color='black', font_weight='bold', alpha=1.0, vmin=0.0, vmax=1.0)
    nx.draw(G, pos, cmap=plt.cm.Greens, node_color=values/1.3+0.15, \
            with_labels=True, node_size=800, font_color='red', font_weight='bold', alpha=1.0, vmin=0.0, vmax=1.0)
    for v, lab in zip(sorted(list(set(values/1.3+0.15)))[::-1], ['slight', 'moderate', 'extensive']):
        plt.scatter([],[], c=[plt.cm.Greens(v)], label=lab)
...
def plot_reg_graph(img_dir, img_idx, edge_list, values, node_ID, edge_labels, plot_norm, img_name):
    ...
    pos = nx.spring_layout(G, seed = 100)
    nx.draw_networkx_edge_labels(G, pos, edge_labels=temp, font_color='red',font_size=12)
    nx.draw(G, pos, cmap=plt.cm.Blues, node_color=values, \
            with_labels=True, node_size=800, font_color='red',font_size=16, font_weight='bold', width=2, vmin=plot_norm.vmin, vmax=plot_norm.vmax*1.2)
    sm = plt.cm.ScalarMappable(cmap=plt.cm.Blues, norm=plot_norm)
...
def plot_reg_graph_new(img_dir, img_idx, edge_list, values, node_ID, edge_labels, plot_norm, img_name):
    import matplotlib.pylab as pylab
    params = {"text.usetex": False,
            'font.size': 16}
    pylab.rcParams.update(params)
    ...
    plt.imshow(img, extent=[-122.0993, -121.7000, 37.2000, 37.4509])
    plt.xlim((-122.0993, -121.7000))
    plt.ylim((37.2000, 37.4509))
    
    pos = nx.get_node_attributes(G,'pos')
    nodes = nx.draw_networkx_nodes(G, pos, cmap=plt.cm.Blues, node_color=values, edgecolors='k', \
            linewidths=1, node_size=200, vmin=plot_norm.vmin, vmax=plot_norm.vmax)
    nx.draw_networkx_edges(G, pos, width=3.0, alpha=0.25)
    sm = plt.cm.ScalarMappable(cmap=plt.cm.Blues, norm=plot_norm)
    position=fig1.add_axes([0.86,0.2,0.04,0.5])
    plt.colorbar(sm, cax=position)
    # plt.axis('equal')
    plt.subplots_adjust(top=1.1, bottom=-0.1, right=1.0, left=0.0, hspace=0, wspace=0)
    plt.savefig(img_dir+"/path_reg_{}_{}.jpg".format(img_name, img_idx))
    plt.close('all')

def plot_cla_graph_new(img_dir, img_idx, graph_data, edge_list, node_ID, edge_labels, img_name, values, num_class):
    import matplotlib.pylab as pylab
    params = {"text.usetex": False,
            'font.size': 16}
    pylab.rcParams.update(params)
    ...
    pos = nx.get_node_attributes(G,'pos')
    nodes = nx.draw_networkx_nodes(G, pos, cmap=plt.cm.Blues, node_color=values/1.3+0.15, edgecolors='k', \
            linewidths=1, node_size=200, vmin=0.0, vmax=1.0)
    nx.draw_networkx_edges(G, pos, width=3.0, alpha=0.25)
    for v, lab in zip(sorted(list(set(values/1.3+0.15)))[::-1], ['slight', 'moderate', 'extensive']):
        plt.scatter([],[], c=[plt.cm.Blues(v)], label=lab)
    plt.legend(markerscale=3.)
    plt.tight_layout()
    plt.subplots_adjust(top=1.0, bottom=0.0, right=1.0, left=0.0, hspace=0, wspace=0)
    plt.savefig(img_dir+"/path_cla_{}_{}.png".format(img_name, img_idx))
...
def plot_system(n_node, n_bridge, n_road, bridge_list, road_list, img_dir):
    ...
    # build graph after earthquake
    plt.figure(figsize=(16, 12))
    # fixed_positions = {0:(0,0), 4:(5,0), 36:(2,-10), 41:(10,-10)}
...
def load_data_case(idx, num_sample, dataset_name, imageset_name):
    ...
    road_list = [(1,6), (2,6), (9,12), (13,20), (13,21), (17,23), \
        (19,26), (23,32), (29,33), (30,34), (33,37), (36,38)]
    # compatiable with bridge_info_v2
    target_node_ID = idx
    node_thre = [0.75]
    node_class = len(node_thre) + 1
    N_iter = 10000
    data_dir, img_dir = './{}/data_{}_v2'.format(dataset_name, target_node_ID), './{}/img_{}_v2'.format(imageset_name, target_node_ID)
...
# def generate_sample(bridge_list, road_list, n_node, n_bridge, n_road, n_sample, random=True):
#     import metis
#     G = nx.Graph()
#     node_ID = [i for i in range(n_node)]
#     G.add_nodes_from(node_ID)
...
def generate_sample(bridge_list, road_list, n_node, n_bridge, n_road, n_sample, random=True):
    ...
    G.add_edges_from(road_list)
    with temp_seed(123):
        sample_targets = np.random.choice(list(G.nodes()), size=n_sample, replace=False)
    return sample_targets
